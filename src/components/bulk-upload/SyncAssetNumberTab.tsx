...
import { HrpBrand } from "@/lib/types";
...
import {
  ...
  NumberSyncRowStatus,
  buildNumberSyncRows,
  fetchExistingAssetsForNumberSync,
} from "@/lib/import/asset-number-sync";
...
const STATUS_META: Record<NumberSyncRowStatus, { label: string; colorClass: string }> = {
  ...
  invalid_number: { label: "No Tidak Valid", colorClass: "bg-slate-100 text-slate-500 border-slate-200" },
  duplicate: { label: "Duplicate", colorClass: "bg-red-50 text-red-700 border-red-200" },
};

export default function SyncAssetNumberTab() {
  ...
  const [rows, setRows] = useState<NumberSyncRow[]>([]);
  ...
  const handleFile = async (f: File) => {
    try {
      ...
      console.error("[Sync No. Aset] gagal membaca file", err);
  ...
  const buildPreview = async () => {
    ...
    try {
      ...
      const existingAssets = await fetchExistingAssetsForNumberSync(companyId);
      const built = buildNumberSyncRows(parsed, existingAssets);
      setRows(built);
      setSelectedKeys(new Set(built.filter((r) => r.status === "ready").map((r) => r.excelRowNumber)));
      setStep("preview");
    } catch (err) {
      console.error("[Sync No. Aset] gagal menyiapkan preview", err);
  ...
  const summary = useMemo(() => {
    ...
    const invalidNumber = rows.filter((r) => r.status === "invalid_number").length;
    const duplicate = rows.filter((r) => r.status === "duplicate").length;
    return { total, ready, same, needsReview, notFound, invalidNumber, duplicate };
  }, [rows]);
  ...
  const toggleRow = (row: NumberSyncRow) => {
  ...
  const handleSync = async () => {
    const selectedRows = rows.filter((r) => selectedKeys.has(r.excelRowNumber) && r.status === "ready" && r.matchedAsset);
    ...
    let success = 0;
    let failed = 0;
    const outcomeRows: { row: NumberSyncRow; ok: boolean; message: string }[] = [];

    for (const row of selectedRows) {
      const asset = row.matchedAsset!;
      try {
        // HANYA field assetNumber yang diupdate — TIDAK PERNAH menyentuh
        // document ID, assetCode, nama, qty, harga, lokasi, perusahaan,
        // divisi, kondisi, foto, Drive file, QR, PIC, finance, maintenance,
        // peminjaman, atau histori.
        await updateDoc(doc(db, "assets", asset.id), {
          assetNumber: row.parsedNumber,
        });
        // Debug sementara — bukti bahwa updateDoc() benar-benar dieksekusi
        // dan berhasil (bukan cuma preview di layar).
        console.log("[Sync Asset Number] updated", {
        ...
        await writeAssetLog({
          ...
          assetCode: asset.assetCode,
          action: "SYNC_ASSET_NUMBER_FROM_EXCEL",
          userUid: currentUserUid,
          userName: currentUserName,
          detail: `No. Aset disinkronkan dari ${asset.assetNumber ?? "-"} menjadi ${row.parsedNumber} (file "${file?.name}" sheet "${sheetName}")`,
        });
        success += 1;
        outcomeRows.push({ row, ok: true, message: "Berhasil disinkronkan" });
      } catch (err) {
        console.error("[Sync No. Aset] updateDoc gagal", { assetCode: asset.assetCode, err });
        failed += 1;
        outcomeRows.push({ row, ok: false, message: "Gagal menyimpan No. Aset" });
      }
      done += 1;
    ...
    const report = [
      ...
      ...rows
        .filter((r) => r.status === "not_found" || r.status === "duplicate" || r.status === "needs_review" || r.status === "invalid_number")
        .map((r) => ({
          ...
          reason:
            r.status === "duplicate"
              ...
              : "Kolom No di Excel kosong/bukan angka valid",
        })),
    ];
    ...
    <div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
          <label className="file-drop py-14">
            ...
            <span className="text-xs text-slate-400">
              ...
              No. Aset yang disinkronkan lewat Kode Aset.
            </span>
            <input
              type="file"
              accept=".xlsx,.xls"
      ...
      {step === "preview" && (
        <div className="space-y-5">
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3">
            ...
            <SummaryCard label="No Tidak Valid" value={summary.invalidNumber} tone="slate" />
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="ml-auto flex items-center gap-2">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-blue-600 to-teal-500 text-white px-4 py-2.5 text-sm font-medium hover:brightness-105 disabled:opacity-50 shadow-md shadow-blue-900/20"
                >
                  Sinkronkan {selectedCount} No. Aset
                </button>
              </div>
            </div>
              ...
              <table className="w-full min-w-[1300px] text-sm">
                <thead className="sticky top-0 bg-slate-50">
                  <tr className="text-left text-slate-500 border-b border-slate-200">
                    ...
                    <th className="px-3 py-2 font-semibold">No Excel</th>
                    <th className="px-3 py-2 font-semibold">Kode Aset Excel</th>
                    <th className="px-3 py-2 font-semibold">Nama Aset Excel</th>
                    <th className="px-3 py-2 font-semibold">Kode Aset Web</th>
                    <th className="px-3 py-2 font-semibold">Nama Aset Web</th>
                    <th className="px-3 py-2 font-semibold">No Existing</th>
                    <th className="px-3 py-2 font-semibold">No Baru</th>
                  ...
                  {rows.map((row) => {
                    ...
                    return (
                      <tr key={row.excelRowNumber} className="border-b border-slate-100 last:border-0 align-top">
                        ...
                        <td className="px-3 py-2 text-slate-500">{row.noExcel || "-"}</td>
                        ...
                        <td className="px-3 py-2 font-mono text-xs text-slate-700 whitespace-nowrap">{row.matchedAsset?.assetCode || "-"}</td>
                        <td className="px-3 py-2 text-slate-500">{row.matchedAsset?.assetName || "-"}</td>
                        <td className="px-3 py-2 text-slate-500">{row.matchedAsset?.assetNumber ?? "-"}</td>
                        <td className="px-3 py-2 font-semibold text-slate-800">{row.parsedNumber ?? "-"}</td>
      ...
      {step === "syncing" && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 text-center space-y-4">
          ...
          <p className="text-sm font-medium text-slate-700">
            Sinkronisasi No. Aset {syncProgress.done} / {syncProgress.total}
          </p>
      ...
      {step === "done" && result && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 text-center space-y-5">
          ...
          <h2 className="text-lg font-semibold text-slate-900">Sinkronisasi No. Aset Selesai</h2>
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 max-w-2xl mx-auto text-left">
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
              <p className="text-xl font-semibold text-slate-700">{summary.invalidNumber}</p>
              <p className="text-xs text-slate-500 mt-0.5">Tidak Valid</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 pt-2">
            ...
            <button
              type="button"
              onClick={() => downloadErrorReportXlsx(reportRows, `Sinkronisasi No Aset - ${file?.name || "sync"}`)}
